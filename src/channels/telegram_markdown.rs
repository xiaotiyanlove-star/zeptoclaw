...
pub fn render_and_chunk_telegram_markdown(content: &str, chunk_size: usize) -> Vec<String> {
    ...
    let open_all_tags = |tags: &Vec<ActiveTag>| -> String {
        tags.iter().map(|t| t.open_tag()).collect::<String>()
    };
        ...
        match event {
            ...
            Event::End(tag_end) => {
                match tag_end {
                    ...
                    TagEnd::Heading { .. } => {
                        if let Some(idx) = active_tags.iter().rposition(|t| matches!(t, ActiveTag::Bold)) {
                    ...
                    TagEnd::BlockQuote => {
                        if let Some(idx) = active_tags.iter().rposition(|t| matches!(t, ActiveTag::Blockquote)) {
                            active_tags.remove(idx);
                    ...
                    TagEnd::CodeBlock => {
                        if let Some(idx) = active_tags.iter().rposition(|t| matches!(t, ActiveTag::Pre)) {
                            active_tags.remove(idx);
                    ...
                    TagEnd::Emphasis => {
                        if let Some(idx) = active_tags.iter().rposition(|t| matches!(t, ActiveTag::Italic)) {
                            active_tags.remove(idx);
                            event_str.push_str(ActiveTag::Italic.close_tag());
                        }
                    },
                    TagEnd::Strikethrough => {
                        if let Some(idx) = active_tags.iter().rposition(|t| matches!(t, ActiveTag::Strikethrough)) {
                            active_tags.remove(idx);
                            event_str.push_str(ActiveTag::Strikethrough.close_tag());
                        }
                    },
                    TagEnd::Link { .. } => {
                        if let Some(idx) = active_tags.iter().rposition(|t| matches!(t, ActiveTag::Anchor(_))) {
            ...
            Event::Text(text) => {
                if in_code_block {
                    ...
                    for (i, piece) in pieces.iter().enumerate() {
                        if i > 0 {
                            if in_spoiler {
                                if let Some(idx) = active_tags.iter().rposition(|t| matches!(t, ActiveTag::Spoiler)) {
                                    active_tags.remove(idx);
    ...
    if !current_chunk.trim().is_empty() {
        chunks.push(format!("{}{}", current_chunk.trim_end(), close_all_tags(&active_tags)));
    }
...
mod tests {
    ...
    fn test_chunking_with_tag_balancing() {
        ...
        for chunk in chunks {
            ...
            assert_eq!(open_count, close_count, "Tags unbalanced in chunk: {}", chunk);
        }
    }
}
